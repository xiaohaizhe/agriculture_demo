...
app = Flask(__name__)
app.config.update(
    MONGO_URI='mongodb://127.0.0.1:27017/agriculture_project'
)
...
def query_by_date(sdate, edate, page, number):
    result = {}
...
def get_price_analyse():
    results = []
    results.append(price_analyse(""))
    for pro in province:
        r = price_analyse(pro)
        if r == None:
            continue
